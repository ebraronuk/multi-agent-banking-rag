...
def test_ner_node_extracts_normally_without_a_pending_request() -> None:
    node = build_ner_node()
...
def test_ner_node_synthesizes_entity_from_bare_answer_when_pending() -> None:
    node = build_ner_node()
...
def test_ner_node_does_not_duplicate_when_regex_already_found_the_entity() -> None:
    node = build_ner_node()
